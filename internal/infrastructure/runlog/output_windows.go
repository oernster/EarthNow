...
// program started without one reads a handle of 0 (Bridge Talk, measured
// 2026-09-14), so everything it writes there is lost.
...
// sendAll points the run's error output at log: first the handle the Go runtime
// looks up for each report it writes, then os.Stderr, which was fixed from that
// handle when the program started.
