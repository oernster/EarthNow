#!/usr/bin/env bash
# Builds the EarthNow macOS DMG for Apple Silicon (DEL-007, release 2). Run on an arm64 Mac
# from the repo root:
#
#   bash builddmg.sh
#
# Flow: place the application icon, wails build for darwin/arm64, stamp the bundle version
# from VERSION, place the licence, codesign the .app (hardened runtime), notarize and staple
# the .app, stage it with ditto, create-dmg, stamp the DMG file icon, sign the DMG, then
...
#
# Ported from SymDiary's port of PigeonPost's builddmg.sh. One difference: the icon step
# copies assets/application-icon.png to build/appicon.png, exactly as build.ps1 does for
# Windows, because build/ is gitignored and holds only generated files. Wails makes the
# bundle's iconfile.icns from it and writes build/darwin itself on the first run for this
# platform, so no Info.plist is committed; the version is stamped into the bundle below.
#
...
#
# Output: EarthNow.dmg in the repo root
set -euo pipefail
...
ICON_MASTER="assets/application-icon.png"
ICON_TARGET="build/appicon.png"
...
#   xcrun notarytool store-credentials EarthNow \
...
NOTARY_PROFILE="${APPLE_KEYCHAIN_PROFILE:-EarthNow}"
...
if [ "${NOTARIZING}" -eq 0 ]; then
    ...
    if ! [[ "${APPLE_APP_PASSWORD}" =~ ${APP_SPECIFIC_PASSWORD_RE} ]]; then
        ...
        echo "App-Specific Passwords). Leave both variables unset instead and store the" >&2
    ...
    # asking notarytool itself is the only reliable probe. A history query costs a couple
    # of seconds against a full wails build; it also proves the credential authenticates
...
section "Placing the application icon"
# The same one master build.ps1 places for Windows (DEL-004), copied fresh every run so
# a changed master can never ship beside an icon made from the old one.
[ -f "${ICON_MASTER}" ] || { echo "error: ${ICON_MASTER} is missing" >&2; exit 1; }
mkdir -p "$(dirname "${ICON_TARGET}")"
cp "${ICON_MASTER}" "${ICON_TARGET}"
...
# Section 4 of the GNU GPL asks that a copy of the licence reach every recipient along
# with the program, so it goes inside the bundle. It has to happen BEFORE signing: a
# file added afterwards is a file Gatekeeper was never told the hash of, so the
...
if [ "${NOTARIZING}" -eq 1 ]; then
    ...
    # Proving the ticket is attached, not merely that stapling was attempted. A notarized
    # build with no ticket on it still launches for whoever built it, then asks the network
    # on someone else's machine and fails there. The DMG gets the same check further down;
    # this one covers the bundle a user copies out of it, which is the copy they run.
...
section "Done"
echo "${DMG_PATH}"
echo "Settings, cache and log live under ~/Library/Caches/${APP_NAME}"
