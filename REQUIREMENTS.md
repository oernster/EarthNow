...
Status: **Baselined on 2026-09-23, with amendments 1 to 42.**
...
| FR-RPL-020 | Must | The top bar shall hold the Play/Pause button, the scrubber, the Now button and the speed button, in that order, after the time window control, taking no height from the globe area (NFR-UX-001). | At 960 by 700 all four show on the top bar's one row (D); the globe area is unchanged (T). | T + D |
...
| FR-RPL-024 | Must | The top bar shall offer a Now button reading "Now" with the tooltip and accessible name "Back to now" (NFR-UX-004). While replaying, activating it shall stop play, return the scrubber to its right end and show the ordinary view of the chosen window (FR-RPL-002). While not replaying it shall be disabled (amendment 42). | Paused at 20 Sep 14:00, pressing Now shows the ordinary view with the scrubber at its end and the button reading "Play replay"; with no replay the Now button is disabled. | T |
...
| NFR-KBD-009 | Must | The Play/Pause button, the scrubber, the Now button and the speed button shall each be one ring stop after the time window, the Now button skipped while disabled; while the scrubber holds focus, Left and Right shall step it by one hundredth of the span, Home and End shall move it to the ends and Space shall play or pause. | Vitest over the ring and the keys. | T |
