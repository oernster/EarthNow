...
- **The settings file reads and writes on its own.** `internal/infrastructure/settings` carries its own capped read and write-then-rename, much as `cache` shares one reader and one writer between the providers' files and the cloud image. Folding settings into those helpers would change its error wording and its temporary file's name for no behaviour the reader sees; the owner judged it fine as it stands (2026-09-24).
